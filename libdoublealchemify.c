...
int	readPDB(char *pdb, char col, int *initialone, int *finalone, int *initialtwo, int *finaltwo, int *nInitialOne, int *nFinalOne, int *nInitialTwo, int *nFinalTwo) {
    ...
    *nInitialTwo = 0;
	...
	if (line[0]=='A'&&line[1]=='T'&&line[2]=='O'&&line[3]=='M') {
	    ...
	    if (input == 1.0) {
		if (*nFinalOne>=MAX_GROUP_SIZE) CLOSE_AND_DIE(f, "too many final atoms")
		finalone[(*nFinalOne)++] = natoms;
	    }
	    if (input == -1.0) {
		if (*nInitialOne>=MAX_GROUP_SIZE) CLOSE_AND_DIE(f, "too many initial atoms")
		initialone[(*nInitialOne)++] = natoms;
	    }
            if (input == 2.0) {
                if (*nFinalTwo>=MAX_GROUP_SIZE) CLOSE_AND_DIE(f, "too many final atoms")
                finaltwo[(*nFinalTwo)++] = natoms;
            }
            if (input == -2.0) {
                if (*nInitialTwo>=MAX_GROUP_SIZE) CLOSE_AND_DIE(f, "too many initial atoms")
                initialtwo[(*nInitialTwo)++] = natoms;
            }
...
/* Does the job									    */
/************************************************************************************/
int	process(FILE *in, FILE *out, int natoms, int *initialone, int *finalone, int *initialtwo, int *finaltwo,
		int nInitialOne, int nFinalOne, int nInitialTwo, int nFinalTwo) {
	...
	bonds = 0;
	for (i=0; i<n; i++)
	    if (couples(p+2*i, 2, initialone, finalone, nInitialOne, nFinalOne)) {
		bonds++;
		wrong = 2*i;
	    }
            if (couples(p+2*i, 2, initialtwo, finaltwo, nInitialTwo, nFinalTwo)) {
                bonds++;
                wrong = 2*i;
            }
	...
	removed = 0;
	for (i=0; i<n; i++)
	    if (couples(p+3*i, 3, initialone, finalone, nInitialOne, nFinalOne))
		    removed++;
            if (couples(p+3*i, 3, initialtwo, finaltwo, nInitialTwo, nFinalTwo))
                    removed++;
	...
	for (i=0; i<n; i++)
	    if (!couples(p+3*i, 3, initialone, finalone, nInitialOne, nFinalOne) && !couples(p+3*i, 3, initialtwo, finaltwo, nInitialTwo, nFinalTwo)) 
	...
	removed = 0;
	for (i=0; i<n; i++)
	    if (couples(p+4*i, 4, initialone, finalone, nInitialOne, nFinalOne))
		    removed++;
            if (couples(p+4*i, 4, initialtwo, finaltwo, nInitialTwo, nFinalTwo))
                    removed++;
	...
	for (i=0; i<n; i++)
	    if (!couples(p+4*i, 4, initialone, finalone, nInitialOne, nFinalOne) && !couples(p+4*i, 4, initialtwo, finaltwo, nInitialTwo, nFinalTwo)) 
    ...
    printf("Writing %i exclusion pairs\n", nInitialOne*nFinalOne+nInitialTwo*nFinalTwo);
    fprintf(out, "%8i !NNB\n", nInitialOne*nFinalOne+nInitialTwo*nFinalTwo);

    /* Give every "initial" atom an exclusion list with all "final" atoms 	 */
    for (i=0; i<nInitialOne; i++)
	for (j=0; j<nFinalOne; j++)
	    write_ints(out, finalone+j, 1, 8);

    for (i=0; i<nInitialTwo; i++)
        for (j=0; j<nFinalTwo; j++)
            write_ints(out, finaltwo+j, 1, 8);
	...
	for (j=0; j<nInitialOne; j++)
		if (a == initialone[j])
			i += nFinalOne;
        for (j=0; j<nInitialTwo; j++)
                if (a == initialtwo[j])
                        i += nFinalTwo;
...
/************************************************************************************/
int couples(int *p, int n, int *initial, int *final, int nInitialOne, int nFinalOne) {
